...
function m_navfunc(color='#222', rightVal='-100%', opacityVal=0){
    m_showHide_Boolean = !m_showHide_Boolean; // !가 들어있으면 오른쪽에 있는 것을 반전해서 왼쪽으로 대입한다는 것임
    ...
    /* m_nav_bg.style.transition = 'right 0.4s, opasity 1s */ /* css에 적은 애니메이션을 여기에 적을 수 있음 */
}

// 클릭 이벤트 시 메뉴 보이기/숨기기
// m_nav_btn.addEventListener('click',()=>{
    // 클릭 시 동적으로 구현하고자 하는 동작이 두개인 경우
    // 위 두상황을 컴퓨터가 인식하는 0, 1로 구분해서 생각한다.
    // 메뉴가 보인다 == 1, 메뉴가 안보인다 == 0
    // 사용자가 처음 접속 시 안보이는 걸로 시작하는지 보이는 걸로 시작하는지에 따라
    // 해당 값을 변수의 초기화값으로 저장한다
    // let m_showHide_Boolean = 0; // 안보이는 걸로 시작한 초기값
    // 사용자의 행동에 따라 변경된 초기값 예측을 위해 조건식으로 작업
    // m_showHide_Boolean==0 ? m_navfunc('#fff', '0%', 1) : m_navfunc()
    // 조건 ? 조건이 참일때 실행 : 조건이 거짓일 때 실행
